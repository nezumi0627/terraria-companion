...
export function looksLikeDescriptionSentence(text) {
  ...
  if (
    ...
    return true
  if (
    /(ことで解除|でオート攻撃|からでも獲得|と時間がリセット|を描いて落下|に触れると消滅|を連続で放つ|に変換して放つ|当たるとバウンド|ダメージを与えることも可能|物が釣れやすく|一定距離進むと消滅|の早い段階で入手|小型の弾を打ち出して|防御を\d+無視|射程が短くなる|マナとMinion|バフは.*持続|武器になる|弾を販売する|消耗品の製作|低空を飛ぶ|難易度によって変動|回収して再設置が可能|光源ペット召喚アイテム)/.test(
      s,
    )
  )
    return true
