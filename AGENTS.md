...
1. **推奨（恒久）**: `workers/cloud-api` を Cloudflare Workers にデプロイし、Variable `CLOUD_API_URL` に **workers.dev（またはカスタム）URL** を設定。`trycloudflare.com` は入れない（Pages に焼き込まれて切れる）
2. **アカウント不要（フォールバック）**: `.github/workflows/cloud-api-tunnel.yml` が Quick Tunnel で API を公開し、`public/cloud-api-url.json` を更新。クライアントは失敗時に URL を再取得する。約4時間ごとに張り直し
